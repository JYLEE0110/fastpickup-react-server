    ...
    <div className="mt-10 px-5 border-3">
      <ul className="flex justify-center">
        ...
        {pageNums.map((num) => (
          ...
          >
            {num === page ? (
              <span className="block px-2 h-8 leading-8 text-[#dc4a51] bg-black rounded">
                {num}
              </span>
            ) : (
              <span className="block px-2 h-8 leading-8 text-white hover:text-[#690] hover:bg-[#2C2A29] rounded transition-all">
                {num}
              </span>
            )}
          </li>
