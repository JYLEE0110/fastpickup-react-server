      ...
      {review.list.map((item, index) => (
        <div key={index} className="border p-4 mb-4 flex">
          <div className="flex-1">
            <h2 className="text-lg font-bold mb-2">{item.reviewTitle}</h2>
            <p className="text-gray-600">주문상품: {item.productName}</p>
            <div className="w-20 h-20 overflow-hidden rounded-full">
              <img
                src={`http://localhost/review/${review.imgName}`}
                className="w-full h-full object-cover"
              />
            </div>
          </div>
          <p className="text-gray-600">{item.registDate.slice(0, 10)}</p>
        </div>
      ))}
      <div>
        <ListPageComponent
